CREATE OR REPLACE PROCEDURE dedup_stuff.superDeduper(
  ingest_table STRING,
  target_table STRING,
  order_column STRING,
  latest_flag STRING,
  unique_columns ARRAY<STRING>,
  measurement_columns ARRAY<STRING>
)
...
DECLARE rxstring_only_unique_columns STRING DEFAULT r'(?i)"(?:' || ARRAY_TO_STRING(ARRAY_CONCAT((SELECT [order_column, latest_flag]), measurement_columns), "|") || r')"(?-i):.+?[,}]';
DECLARE rxstring_only_measurement_columns STRING DEFAULT r'(?i)"(?:' || ARRAY_TO_STRING(ARRAY_CONCAT((SELECT [order_column, latest_flag]), unique_columns), "|") || r')"(?-i):.+?[,}]';
...
CREATE TEMP TABLE update_staging AS (
  ...
  SELECT TRUE AS """ || latest_flag || """, i.* FROM """ || ingest_table || """ i, """ || target_table || """ m
  WHERE
